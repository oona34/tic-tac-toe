...
const gameBoard = ((rows = 3, columns = 3) => {
    ...
    // Create a 2D array to represent the game board, with each cell created by the cellCreator function
    const boardArray = Array.from({ length: rows }, () => Array.from({ length: columns }, () => cellCreator()));
    ...
    const printBoard = () => {
        const boardWithCellValues = boardArray.map((row) => row.map((cell) => cell.getValue()));
    ...
    const getAssociatedBoardLines = (row, column) => {
        ...
        const getValueOrOffboard = (row, column) => {
            if (row >= 0 && row < rows && column >= 0 && column < columns) {
                return boardArray[row][column].getValue();
            } else {
                return 'offboard';
            }
        }
        ...
        const leftDiagonal = [getValueOrOffboard(row - 1, column - 1), getValueOrOffboard(row + 1, column + 1)];
        const rightDiagonal = [getValueOrOffboard(row + 1, column - 1), getValueOrOffboard(row - 1, column + 1)];
        const horizontal = [getValueOrOffboard(row, column - 1), getValueOrOffboard(row, column + 1)];
        const vertical = [getValueOrOffboard(row - 1, column), getValueOrOffboard(row + 1, column)];
    ...
    // Return an object with the game board and functions for getting the board, printing the board, and getting the values of the cells in the lines relative to a given cell
    return { boardArray, getBoard, printBoard, getAssociatedBoardLines };
})();
...
const gameFlowController = (() => {
    ...
    const players = [playerFactory("Player A", "X"), playerFactory("Player B", "O")];
    ...
    const isWinningPlay = (row, column) => {
        ...
        for (let [lineName, lineArray] of Object.entries(cellLines)) {
            if (lineArray.filter(value => value === getActivePlayer().getSymbol()).length === 2) {
                return true;
            }
        }
        return false;
    ...
    const playRound = (row, column) => {
        const cell = board.boardArray[row - 1][column - 1];
